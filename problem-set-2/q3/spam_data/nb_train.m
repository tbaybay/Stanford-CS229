...
for i = 1:length(training_set_sizes)
    ...
    for j = 1:numTokens
        phi_spam(j) = (sum((trainMatrix(:, j) > 0).*(trainCategory(:) == 1)) + 1)./(sum(trainCategory == 1) + 2);
        phi_nonspam(j) = (sum((trainMatrix(:, j) > 0).*(trainCategory(:) == 0)) + 1)./(sum(trainCategory == 0) + 2);
    end
    
    test_errors(i) = nb_test(phi_y, phi_spam, phi_nonspam);
